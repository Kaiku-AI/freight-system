...
**状态节点**（订舱/放舱/装箱/舱单/提单/开船确认）——用可空时间戳记录节点完成，列表可展示进度：

| 字段 | 中文 | 类型 |
|---|---|---|
| booking_confirmed_at | 订舱确认 | timestamp null |
| space_released_at | 放舱确认 | timestamp null |
| loaded_at | 装箱确认 | timestamp null |
| manifest_confirmed_at | 舱单确认 | timestamp null |
| bl_confirmed_at | 提单确认 | timestamp null |
| sailed_at | 开船确认 | timestamp null |
...
| POST | `/api/jobs/{id}/nodes/{node}` | 标记节点完成（如订舱确认/放舱确认），可选增强 |
...
| 路由 | 页面 | 对应需求 |
|---|---|---|
| `/login` | 登录页（固定 test 账号，无鉴权） | 需求 4 |
| `/` | **主页 / 模块导航**：各功能模块卡片入口；卡片均可点，已做的进入真实页面，未做的跳 `/unavailable` 占位页 | 入口 |
| `/unavailable` | **暂未开放占位页**：未开放模块点进来都到这，仅显示「暂未开放」提示，无列表数据 | 占位 |
| `/jobs` | **作业列表**：筛选条 + 表格（作业号/委托人/船名·航次/MB·L No./起运港/目的地/ETD/出运状态/拖车/报关/提单）+ 「新建」「删除」按钮（删除带二次确认） | 需求 2+3：查询/读取/删除 |
| `/jobs/new` | **新建整箱作业**：基本信息 + 服务标志 + 托单信息（必填项黄底/红星，字段以 §4 为准）；其余子页签（集装箱/MB·L/HB·L 等）暂未开放 | 需求 1：写入 |
...
交互参考手册：必填项黄底带 `*`；列表可按条件搜索；明细页能看到「订舱确认→放舱→装箱→舱单→提单→开船」节点进度。
