...
编辑表单的「动作」下拉 5 项：点「订舱」弹轻提示「订舱指令已发送」（1.8s 自动消失）；其余「退舱 / 锁定作业 / 作废 / 转工作号」弹「暂不可用」提示（纯撑场面，便于录演示）。

**纯演示、零副作用**：不接真实 EDI、不生成回执号、不回填表单、不动后端、不改字段/路由——只发一句提示。下拉与提示在 `app/jobs/_components/BookingAction.tsx`（无 props，自管菜单开合与提示计时）。
