...
**只在编辑态可点**：订舱是对「已落库的作业单」发起的动作（新建时作业单尚未保存、信息也未填全），故**新建**表单的「动作 ▾」灰显不可点（与同栏其它装饰按钮一致），进入**编辑**才变为真实下拉。由 `JobForm` 的 `job` prop 派生：有 `job`=编辑→真实组件，无=新建→`ToolbarGhost` 占位。

编辑表单的「动作」下拉里，「订舱」为唯一真实可用项；其余「退舱 / 锁定作业 / 作废 / 转工作号」点击只弹「暂不可用」提示（纯撑场面，便于录演示）。

点「订舱」走一条**纯前端模拟**流程：弹窗「正在向船公司发送订舱 EDI…」（约 1.3s 模拟时延）→ 收到模拟回执（生成船东订舱号 `SO+YYYYMMDD-4位流水`）→ 询问是否回填。

**关键决策——只回填不落库**：确认后仅把订舱号写入 `so_no`、勾选「订舱确认」`booking_confirmed` 到表单 state，是否保存仍由用户点工具栏「保存」统一决定。这样保存口径单一、无歧义，避免把用户其它未保存改动一并自动提交；弹窗文案明确提示需点【保存】方才生效。

不接真实 EDI、不动后端、不改字段/路由。回执号生成函数 `makeBookingReceipt()` 放 `app/jobs/_components/fields.ts`（纯函数，有 Vitest 覆盖）；下拉与弹窗在 `app/jobs/_components/BookingAction.tsx`。
