...
- **2026-06-10** — 增量功能：编辑表单工具栏「动作 ▾」加**模拟订舱**（演示用）。新真实下拉组件 `app/jobs/_components/BookingAction.tsx`：下拉 5 项（订舱/退舱/锁定作业/作废/转工作号）全可点，仅「订舱」走真实流程，其余点击弹「暂不可用」提示（1.8s 自动消失，撑场面便于录 demo）。订舱流程为**纯前端模拟**：弹窗「正在向船公司发送订舱 EDI…」（1.3s 模拟时延）→ 收到模拟回执（`makeBookingReceipt()` 生成船东订舱号 `SO+YYYYMMDD-4位流水`）→ 询问回填。关键决策一：**只回填不保存**——确认后仅把订舱号写入 `so_no`、勾选 `booking_confirmed` 到表单 state，是否落库仍由顶部「保存」统一触发（避免把用户其它未保存改动一并自动提交，保存口径单一无歧义；弹窗文案明确提示需点【保存】方生效）。关键决策二：**仅编辑态可点**——订舱是对「已落库作业单」发起的动作，新建时作业单未保存、信息未填全，故新建表单「动作 ▾」灰显占位（`ToolbarGhost`），进入编辑才换真实组件；由 `JobForm` 的 `job` prop 派生。不接真实 EDI、不动后端、不改字段/路由。回执生成函数放 `fields.ts`（纯函数便于单测）。回归：`__tests__/jobForm.test.ts` 加 2 例（回执号格式 + 随机性），Vitest 14 绿、eslint 绿；浏览器 QA：新建表单「动作 ▾」灰显不可点、订舱→发送→回执→回填全程通、so_no 与订舱确认正确回填、console 无报错；DESIGN.md §13.1 同步。备注：沙箱 Linux 跑 Vitest 需补 rolldown 的 linux-arm64 native binding（仅本地 node_modules，未入 package.json）。
