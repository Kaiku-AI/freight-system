...
- **2026-06-10** — 增量功能：编辑表单工具栏「动作 ▾」下拉（演示用），随后**按反馈简化**为纯提示。新组件 `app/jobs/_components/BookingAction.tsx`：下拉 5 项（订舱/退舱/锁定作业/作废/转工作号），点「订舱」弹轻提示「订舱指令已发送」、其余弹「暂不可用」（均 1.8s 自动消失，撑场面便于录 demo）。**纯演示、零副作用**：不接真实 EDI、不生成回执号、不回填表单、不动后端、不改字段/路由（初版曾做「发送→回执→回填 so_no+勾订舱确认」的弹窗流程，按用户反馈砍掉，连带删 `fields.ts` 的 `makeBookingReceipt`/`BookingReceipt` 及其单测）。关键决策：**仅编辑态可点**——订舱是对「已落库作业单」发起的动作，新建时作业单未保存、信息未填全，故新建表单「动作 ▾」灰显占位（`ToolbarGhost`），进入编辑才换真实组件；由 `JobForm` 的 `job` prop 派生。回归：Vitest 12 绿、eslint 绿；浏览器 QA：新建「动作 ▾」灰显不可点、编辑下拉点「订舱」弹「订舱指令已发送」、console 无报错；DESIGN.md §13.1 同步。备注：沙箱 Linux 跑 Vitest 需补 rolldown 的 linux-arm64 native binding（仅本地 node_modules，未入 package.json）。
