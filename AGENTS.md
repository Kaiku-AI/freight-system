...
- **视觉令牌单一数据源**：配色取自 Penpot 视觉稿，统一定义在 `app/globals.css` 的 `@theme`（Tailwind v4 命名色 → 自动生成 `bg-/text-/border-` 等工具类，语义名 `brand`/`brand-dark`/`brand-soft`/`ink`/`body`/`muted`/`faint`/`line`/`line-strong`/`canvas`/`field`/`required`/`star`）。全站只用这套语义色，不再裸用 `zinc-/amber-/emerald-` 等默认色阶；出运状态胶囊配色集中在 `fields.ts` 的 `statusBadgeClass`。改配色只动 `@theme`。
  > **补充（2026-06 界面对齐云海，详见根目录 `UI.md`，以下为对上述几条的增量，不改原约定边界）**：① 侧栏顶层导航实际升级为 `SIDEBAR_ITEMS`（云海左栏 17 个模块**平铺无分组**，仅「海运出口」→`/`，其余→`/unavailable`；海运出口在 `/` 与 `/jobs*` 高亮；图标仍按 `item.key` 由 `components/NavIcons.tsx` 渲染线性 SVG，勿用 emoji）。② 主页功能网格实际升级为云海模块面板 6 组，仅 job-new/job-list 真实，组名统一主色蓝，`groupedModules()` 返回 `{group,items}`，未开放跳 `/unavailable`。③ 整箱页签实际对齐云海 9 个，仅 托单信息 真实，未实现页签仍在本卡片内显示「暂未开放」占位、不跳走不丢表单；占位 icon 用白底灰色细描边，不用浅蓝实底。④ 顶部为蓝色软件栏（云朵 logo）+ 其下**橙色页签条** `TOP_TABS`「模块 / 工作台 / 最近打开」（仅「模块」真实、激活=深色块，右侧灰字当前页名 `crumbLabel`）；侧栏在页签条下方、激活项加左 3px 主色竖条；回模块导航主页的入口是顶部页签条「模块」与侧栏「海运出口」。⑤ 模块页工具栏里真实可用的按钮（保存/放弃/编辑/新建/删除/关闭）各页自渲染，其余云海工具按钮用 `_components/Toolbar.tsx` 的 `ToolbarGhost`（`disabled`、扁平文字、本期不可点）并排展示；整箱新建/明细共用 `_components/JobWindow.tsx` 窗体（蓝标题栏 + 顶级工具栏 + 主页签 + 次级工具栏槽）；两级工具栏保持单行横向排列，必要时 `overflow-x-auto`，次级工具栏用浅橙底。⑥ `@theme` 增令牌 `topbar`/`topbar-dark`（顶栏蓝渐变）、`accent`/`accent-soft`（云海橙：页签条/操作链接）、`panel`/`panel-line`（表单分块淡蓝实底 + 面板内描边）；`required`/`required-line` 为云海米黄。⑦ 表单分块为淡蓝实底块（块内粗体标题，无标题条）、字段「标签左输入右」密排、基本信息与服务标志同块；主页/列表/工具栏去大圆角阴影，统一平面 `border` + `rounded`(4px)、工具栏扁平文字按钮。⑧ 登录页标语不出现 AI 字样；分支机构代码不预填 `YH`，分支机构/部门/模拟登录/二维码仅作装饰，不参与登录。⑨ 订舱动作弹窗使用云海式方角系统窗（蓝色标题栏、白底内容、4px 圆角、细描边），文案只表达「订舱指令已发送」，不声称真实订舱成功、不回填业务字段。改界面结构/组件样式先读 `UI.md`。
...
### 浏览器自测（前端 Phase 必做）
后端起在本地后，用 Codex in Chrome 打开 `localhost`：
